...
namespace LeibingerAPI
{
    public class LeibingerStatus : Leibinger
    {
        ...
        // Status Commands (these will return data)
        private readonly string statusCMD = "^0?RS\r"; //get a string of tab delimited parameters indicating status of the printer
        ...
        private readonly string setEncoderCMD = "^0=PR5"; //dont forget to add \r
        ...
        public double InternalEncoderSpeed // returns or sets the speed setting of the internal software encoder in the printer, measured in meters/minute.
        {
            get
            {
                double returnedIntVal;
                string printerStatus = SendCMD(statusCMD);
                string[] parsedStatus = printerStatus.Split('\t');
                Double.TryParse(parsedStatus[4], out returnedIntVal);
                _actSpeed = returnedIntVal / 10;
                return _actSpeed;
            }
            set
            {
        ...
        public int MachineState
        {
            get
            {
                ...
                string printerStatus = SendCMD(statusCMD);
                string[] parsedStatus = printerStatus.Split('\t');
                Int32.TryParse(parsedStatus[1], out _machineState);
